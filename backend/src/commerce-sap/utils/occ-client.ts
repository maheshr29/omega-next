...
import type { OccTokenResponse } from "@Commerce-sap/interfaces/occ.types";
...
async function rawFetch(
  url: URL,
  token: string,
  _opts: OccRequestOptions,
  signal: AbortSignal,
) {
  // `revalidateSeconds` / `tags` from OccRequestOptions are honoured by the
  // backend CacheStore wrapper in phase 6, not at the raw fetch level.
  return fetch(url, {
    headers: {
