...
type Params = { parent: string; slug: string };
...
export default async function CategoryPage({
  ...
  const [homePage, data] = await Promise.all([
    ...
    bff.search
      .categoryContent({ q: slug, rows: 24 })
      .catch((err: unknown) => {
  ...
  console.log("categoryContent.response", JSON.stringify(data, null, 2));
  ...
  const parentLabel = humanize(parent);
      ...
      <main className="flex-1 bg-white">
        <div className="mx-auto w-full max-w-7xl px-6 pt-6 pb-16">
          <nav className="mb-6 text-[13px] text-zinc-600">
            ...
            <Link href={`/${parent}`} className="hover:text-zinc-900">
              {parentLabel}
            </Link>
            <span className="mx-2 text-zinc-400">|</span>
